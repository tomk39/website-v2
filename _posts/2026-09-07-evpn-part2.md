...
│
├── ESI
│   ├── Identifies the shared Ethernet Segment
│   └── Advertised in: Type 1, Type 2, and Type 4 routes
│
├── DF Election
│   ├── Determines which Leaf forwards BUM traffic
│   └── Uses: Type 4 — Ethernet Segment Route
│
├── Aliasing (All-Active)
│   ├── Enables ECMP for known unicast traffic
│   └── Uses:
│       ├── Type 1 — A-D per EVI
│       ├── Type 1 — A-D per ES
│       └── Type 2 — MAC/IP Advertisement
│
├── Backup Path (Single-Active)
│   ├── Provides standby forwarding for known unicast
│   └── Uses:
│       ├── Type 1 — A-D per EVI
│       ├── Type 1 — A-D per ES
│       └── Type 2 — MAC/IP Advertisement
│
└── Split Horizon
    ├── Prevents loops back toward the same Ethernet Segment
    └── Uses: Type 1 — A-D per ES
```
